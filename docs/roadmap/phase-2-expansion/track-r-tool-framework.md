...
- Tool coordinator and handler interfaces live in `packages/agent-runtime-tools/src/coordinator`.
- Hook executor + hook config live in `packages/agent-runtime-tools/src/hooks`.
- Auto-approval + policy helpers live in `packages/agent-runtime-tools/src/approval`.

## Remaining Work
- Finish wiring coordinator/hook flows into runtime execution paths.
- Align documentation + tests for hook cancellation and auto-approval flows.
