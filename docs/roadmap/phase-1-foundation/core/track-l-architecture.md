...
- `agent-runtime-core`, `agent-runtime-control`, `agent-runtime-memory`, `agent-runtime-persistence`,
  `agent-runtime-sandbox`, `agent-runtime-telemetry`, `agent-runtime-tools`, and `agent-runtime-vision` exist.
- `agent-runtime` facade re-exports tools/control/memory/telemetry, but execution + persistence still come
  from the monolith.
- Execution plane extraction and artifacts/checkpoint manager consolidation remain outstanding.

## Open Items
- Create `agent-runtime-execution` and move orchestrator/executor/pipeline modules.
- Move artifacts + checkpoint manager into `agent-runtime-persistence` and re-export via facade.
- Finish facade wiring and dependency enforcement after package cutover.
