...
- `ToolOutputSpooler` contract in `agent-runtime-core` with implementation in `agent-runtime/src/spooling`.
- Execution pipeline spools large outputs and truncates with metadata references.
- Tests in `packages/agent-runtime/src/__tests__/toolOutputSpooling.test.ts`.
...
- Create branch: `feature/track-m3-tool-spooling`
