...
- Tool registry views and isolation helpers live in `agent-runtime-tools` and `agent-runtime/src/tools/workbench`.
- Agent manager creates filtered registries for subagents with allowlists.
- Isolation tests exist in `packages/agent-runtime/src/__tests__/workbenchRegistry.test.ts`.
...
- Create branch: `feature/track-m4-registry-isolation`
