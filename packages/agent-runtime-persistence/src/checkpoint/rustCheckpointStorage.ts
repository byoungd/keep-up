/**
 * Rust-backed Checkpoint Storage
 *
 * Persists checkpoints to disk via the native storage engine, using MessagePack
 * records with optional delta encoding.
 */
...
export interface RustCheckpointStorageConfig {
  rootDir: string;
...
export class RustCheckpointStorage implements ICheckpointStorage {
  ...
  constructor(config: RustCheckpointStorageConfig) {
    ...
    this.engine = config.engine ?? createStorageEngine(this.rootDir);
  }
  ...
  async delete(id: string): Promise<boolean> {
    ...
    this.engine.deleteCheckpoint(entry.fileName);
  ...
  private async readRecord(fileName: string): Promise<StoredCheckpointRecord> {
    const payload = this.engine.loadCheckpoint(fileName);
    if (!payload) {
      throw new Error(`Missing checkpoint record: ${fileName}`);
    }
