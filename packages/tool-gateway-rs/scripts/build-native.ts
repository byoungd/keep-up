...
function getLibraryName(): string {
  if (process.platform === "win32") {
    return "tool_gateway_rs.dll";
  }
  if (process.platform === "darwin") {
    return "libtool_gateway_rs.dylib";
  }
  return "libtool_gateway_rs.so";
}

// biome-ignore lint/suspicious/noConsole: Build script output
console.log(`Building tool-gateway-rs (${isRelease ? "release" : "debug"})...`);
...
const destination = path.join(distDir, "tool_gateway_rs.node");
