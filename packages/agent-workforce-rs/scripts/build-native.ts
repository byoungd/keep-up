...
function getLibraryName(): string {
  if (process.platform === "win32") {
    return "agent_workforce_rs.dll";
  }
  if (process.platform === "darwin") {
    return "libagent_workforce_rs.dylib";
  }
  return "libagent_workforce_rs.so";
}
...
const destination = path.join(distDir, "agent_workforce_rs.node");
