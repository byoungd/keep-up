...
export type SpanList = Array<{
  blockId: string;
  start: number;
  end: number;
}>;

export type BlockTextProvider = (blockId: string) => string | null;

export type ReferenceAnchorBuildOptions = {
  docId?: string;
  createdAtRev?: string;
};
...
export function buildReferenceAnchors(
  spans: SpanList,
  getBlockText: BlockTextProvider,
  options: ReferenceAnchorBuildOptions = {}
): ReferenceAnchor[] {
  const anchors: ReferenceAnchor[] = [];
  for (const span of spans) {
    const blockText = getBlockText(span.blockId) ?? "";
    if (!blockText) {
      continue;
    }
    ...
    anchors.push({
      ...
      prefixText,
      suffixText,
      createdAtRev: options.createdAtRev ?? "",
    });
  }
  return anchors;
}
