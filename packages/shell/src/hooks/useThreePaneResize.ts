...
export function useThreePaneResize({
  ...
  const expandLeft = React.useCallback(
    (targetSize?: number) => {
      if (!isLeftCollapsed && !targetSize) {
        return;
      }

      let targetWidth = prevLeftWidth < minSizes[0] ? minSizes[0] : prevLeftWidth;

      if (targetSize) {
        if (targetSize >= minSizes[0]) {
          targetWidth = targetSize;
          setPrevLeftWidth(targetSize);
        } else if (targetSize > 0) {
          targetWidth = minSizes[0];
          setPrevLeftWidth(targetWidth);
        }
      }
    ...
    [isLeftCollapsed, prevLeftWidth, minSizes]
  );

  const collapseLeft = React.useCallback(() => {
    if (!isLeftCollapsed) {
      setPrevLeftWidth(leftWidth);
      setLeftWidth(0);
      setIsLeftCollapsed(true);
