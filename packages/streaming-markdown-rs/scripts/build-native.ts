...
function getLibraryName(): string {
  if (process.platform === "win32") {
    return "streaming_markdown_rs.dll";
  }
  if (process.platform === "darwin") {
    return "libstreaming_markdown_rs.dylib";
  }
  return "libstreaming_markdown_rs.so";
}
...
const destination = path.join(distDir, "streaming_markdown_rs.node");
