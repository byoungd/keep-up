...
function getLibraryName(): string {
  if (process.platform === "win32") {
    return "anchor_relocation_rs.dll";
  }
  if (process.platform === "darwin") {
    return "libanchor_relocation_rs.dylib";
  }
  return "libanchor_relocation_rs.so";
}
...
const destination = path.join(distDir, "anchor_relocation_rs.node");
