...
function getLibraryName(): string {
  if (process.platform === "win32") {
    return "vector_similarity_rs.dll";
  }
  if (process.platform === "darwin") {
    return "libvector_similarity_rs.dylib";
  }
  return "libvector_similarity_rs.so";
}
...
const destination = path.join(distDir, "vector_similarity_rs.node");
