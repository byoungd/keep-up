...
function getLibraryName(): string {
  if (process.platform === "win32") {
    return "workspace_session_rs.dll";
  }
  if (process.platform === "darwin") {
    return "libworkspace_session_rs.dylib";
  }
  return "libworkspace_session_rs.so";
}
...
const destination = path.join(distDir, "workspace_session_rs.node");
