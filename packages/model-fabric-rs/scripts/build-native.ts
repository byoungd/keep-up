...
function getLibraryName(): string {
  if (process.platform === "win32") {
    return "model_fabric_rs.dll";
  }
  if (process.platform === "darwin") {
    return "libmodel_fabric_rs.dylib";
  }
  return "libmodel_fabric_rs.so";
}

// biome-ignore lint/suspicious/noConsole: Build script output
console.log(`Building model-fabric-rs (${isRelease ? "release" : "debug"})...`);
...
const destination = path.join(distDir, "model_fabric_rs.node");
